...
public abstract class OpenIGTServer {
	...
	STOPPED, LISTENING, CONNECTED, DISCONNECTED
    }; // possible server states

    ServerStatus currentStatus = ServerStatus.STOPPED; // start as stopped
						       // status
	...
	try {
	    ...
	    errorManager.error("OpenIGTServer Could not listen on port: "
		    + this.port, e, ErrorManager.OPENIGTSERVER_IO_EXCEPTION);
	    throw e;
	}
    }
	...
	public void run() {
		...
		while (getKeepAlive()) {
			...
			System.out
				.println("Before waiting for another client, waiting for currnt client to get disconnected");
			while (getServerThread().getAlive() != false) {
			    // wait here until client gets disconnected
			    ThreadUtil.wait(200);
			}
			System.out.println("IGTLink client Disconnected");
	...
	try {
	    ...
	    System.out.println("IGTLink Server Waiting for connection");
	    setServerThread(new ServerThread(socket.accept(), this));
	    getServerThread().start();
	    // getServerThread().setAlive(true);
	...
	if (getServerThread() != null) {
	    ...
	    try {
		// TODO before sending message it should be packed, should this
		// be done here or in Message itself?
     ...
     * 
     * @param serverThread
     *            serverThread managing connection of client where does come
     *            from the message
     *** 
     * @return the message Handler
     */
    public abstract MessageHandler getMessageHandler(Header header,
	    byte[] bodyBuf, ServerThread serverThread);
