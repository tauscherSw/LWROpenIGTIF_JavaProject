	...
	do {
		...
		if (!openIGT_Queue.isEmpty()) {
			...
			try {
			    ...
			    if (!res)
				errorManager.error(
					"ResponseQueueManager ResponseHandler.performResponse result "
						+ res, new Exception(
						"perform response pb "),
					ErrorManager.RESPONSE_PB_RESULT);
			} catch (CrcException c) {
			    errorManager.error("PB responseHandler ", c,
				    ErrorManager.RESPONSE_CRC_EXCEPTION);
			} catch (Exception e) {
			    errorManager.error("PB responseHandler ", e,
				    ErrorManager.RESPONSE_EXCEPTION);
			} finally {
			    Log.debug("ResponseQueueManager ResponseHandler.performResponse OK");
			}
		    } else {
			errorManager.error(
				"ResponseQueueManager responseHandler null",
				new Exception("performResponse error"),
				ErrorManager.RESPONSE_ERROR);
		    }
		}
	    } catch (InterruptedException e) {
		errorManager.error("PB responseHandler ", e,
			ErrorManager.RESPONSE_INTERRUPTED_EXCEPTION);
	    }
	} while (alive && res);
    }
