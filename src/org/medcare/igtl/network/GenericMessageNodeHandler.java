...
public class GenericMessageNodeHandler {
    ...
    public OpenIGTMessage perform(String messageType, Header head, byte[] body,
	    IOpenIgtPacketListener node) throws Exception {
	// Log.debug("perform messageType : " + messageType);
	// Log.info("Recived IGTLink packet, header="+head+" body="+new
	// ByteList( body));
	// TODO - GSF: Need to add complete set of new IGTLInk commands for BRP
	// robot
	...
	if (messageType.equals("TRANSFORM")) {
	    ...
	    TransformNR t = new TransformNR(position, rotation);
	    // TODO Nirav- This seems wrong, it should be calling
	    // getTxTransform() changing it
	    node.onRxTransform(openIGTMessage.getDeviceName(), t);
	    ...
	    node.onRxDataArray(openIGTMessage.getDeviceName(),
		    datMesg.getDataMatrix());// this is a non functional stub
	    ...
	    /*
	     * Log.error("This method is not complete"); DataArrayMessage
	     * datMesg = new DataArrayMessage(head, body); openIGTMessage
	     * =(OpenIGTMessage)datMesg;
	     * Log.debug("Received String Message: Header=" +
	     * datMesg.getHeader() + " and Data=" + datMesg.getBody());
	     */
	    // For string message we have to skip first 4 bytes as 0-1:ENcodinng
	    // and 1-2:Length
	    // currently using only US-ASCII encoding and we don't need length
	    String msgBody = new String(body, 4, body.length - 4, "US-ASCII");

	    node.onRxString(openIGTMessage.getDeviceName(), msgBody);// this is
								     // a non
								     // functional
								     // stub
	    ...
	    node.onGetStatus(openIGTMessage.getDeviceName());// this is a non
							     // functional stub
	} else {
