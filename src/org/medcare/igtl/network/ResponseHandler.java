 ...
 * methods must be adapted corresponding to the need of each use User must adapt
 * Capability list, to be able to send the right capability list and to have a
 * correct error management for incorrect capability request User must use
 * SetStatus method so it value correspond to the real status of the client.
...
public abstract class ResponseHandler {
	...
	manageError("ResponseHandler Message Type not Found " + messageType,
		new Exception("ResponseHandler Message Type not Found"),
		ErrorManager.RESPONSE_NOT_MANAGED);
	return false;
    }
     ...
     * @return True if response job performed successfully
     * @throws Exception
     *             CrcException will be thrown at crc check during message
     *             unpacking Other exceptions can be thrown
     */
     ...
     *            The error Code
     */
    public abstract void manageError(String message, Exception exception,
	    int errorCode);
