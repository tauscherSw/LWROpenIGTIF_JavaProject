...
public class GenericIGTLinkServer extends OpenIGTServer implements
	...
	// TODO Auto-generated method stub
	return new GenericServerResponseHandler(header, bodyBuf, serverThread,
		this);
    }

    /**
     * This method will be called by the IGT server when a TransformNR is
     * received. Supports: TransformNR QTRANS QTransformNR POSITION
     * 
     * @param name
     *            The string in the 'NAME' field of the IGT packet
     * @param t
     */
	...
	if (listeners.size() != 1) {
	    throw new RuntimeException(
		    "There can be only one listener for this packet type.");
	}
     ...
     * 
     * @param name
     *            A string of what type of data to get
     * @param body
     ...
     *            the image
     * @param t
     *            A TransformNR of where the image is
     */
	...
	public void onStringMessage(StringMessage msg) {
	    // by pass Queue if its STOP or EMERGENCY message
	    // if( msg.getMessage() == "STOP" || msg.getMessage() ==
	    // "EMERGENCY"){
	    // sendMessage(msg);
	    // }else{
	    messageQueue.add(msg);
	...
	public void run() {
		...
		while (getServerThread() == null
			|| getServerThread().isAlive() == false) {
		    messageQueue.clear(); // keep cleaning the message Queue
					  // till Clinet connects
		    try {
			Thread.sleep(500);
