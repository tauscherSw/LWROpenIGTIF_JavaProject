	...
	do {
		...
		if (!openIGT_Queue.isEmpty()) {
			...
			try {
			    ...
			    if (!res)
				errorManager.error(
					"PB openIGTClient.sendBytes  ",
					new Exception("sendBytes error"),
					ErrorManager.REQUEST_RESULT_ERROR);
			} catch (Exception e) {
			    errorManager
				    .error("RequestQueueManager openIGTClient.sendBytes ",
					    e, ErrorManager.REQUEST_EXCEPTION);
			} finally {
			    Log.debug("RequestQueueManager SendBytes OK");
			}
		    } else {
			errorManager.error(
				"PB openIGTClient openIGTRequest null",
				new Exception("sendBytes error"),
				ErrorManager.REQUEST_ERROR);
		    }
		}
	    } catch (InterruptedException e) {
		errorManager.error("RequestQueueManager PB dans Thread", e,
			ErrorManager.REQUEST_INTERRUPT_EXCEPTION);
	    }
	} while (alive && res);
    }
