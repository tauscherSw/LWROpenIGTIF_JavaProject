...
public abstract class OpenIGTClient extends Thread {
     ...
     * 
     **************************************************************************/
    public OpenIGTClient(String host, int port, ErrorManager errorManager)
	...
	try {
		...
		if (ret_read > 0) {
		    ...
		    if (ret_read > 0) {
			responseQueue.addResponse(getResponseHandler(header,
				bodyBuf));
		    }
		}
	    } while (isConnected() && ret_read >= 0);
	    Log.debug("Client thread exited! Connected state=" + isConnected()
		    + " ret_read = " + ret_read);
	} catch (UnknownHostException e) {
	    errorManager.error("OpenIGTClient Don't know about host" + host, e,
		    ErrorManager.OPENIGTCLIENT_UNKNOWNHOST_EXCEPTION);
	} catch (IOException e) {
	    errorManager.error(
		    "OpenIGTClient Couldn't get I/O for the connection to: "
			    + host + " port " + port, e,
		    ErrorManager.OPENIGTCLIENT_IO_EXCEPTION);
	} finally {
	    try {
		outstr.close();
	    } catch (IOException e) {
		errorManager.error("OpenIGTClient PB outstr.close", e,
			ErrorManager.OPENIGTCLIENT_IO_EXCEPTION);
	    }
	    try {
		instr.close();
	    } catch (IOException e) {
		errorManager.error("OpenIGTClient PB instr.close", e,
			ErrorManager.OPENIGTCLIENT_IO_EXCEPTION);
	    }
	    try {
		socket.close();
	    } catch (IOException e) {
		errorManager.error("OpenIGTClient PB socket.close", e,
			ErrorManager.OPENIGTCLIENT_IO_EXCEPTION);
	    }
	    this.interrupt();
	}
	...
	try {
	    ...
	    if (outstr == null)
		errorManager.error("OpenIGTClient sendBytes PB outstr null", e,
			ErrorManager.OPENIGTCLIENT_EXCEPTION);
	    errorManager.error(
		    "OpenIGTClient Exception while sendBytes to socket", e,
		    ErrorManager.OPENIGTCLIENT_EXCEPTION);
	    return false;
	}
	return true;
     ...
     * 
     * @param bodyBuf
     *            byte array of the body of the message received
     *** 
     * @return the response Handler
     */
    public abstract ResponseHandler getResponseHandler(Header header,
	    byte[] bodyBuf);
