...
public class MessageQueueManager extends Thread {
	...
	do {
		...
		if (!getOpenIGT_Queue().isEmpty()) {
		    ...
		    MessageHandler messageHandler = (MessageHandler) getOpenIGT_Queue()
			    .poll();
			...
			try {
			    ...
			    if (!res)
				errorManager
					.error("MessageQueueManager messageHandler.performRequest result "
						+ res,
						new Exception(
							"messageHandler.performReques Pb Result"),
						ErrorManager.MESSAGE_PB_RESULT);
			} catch (CrcException c) {
			    errorManager.error("PB messageHandler ", c,
				    ErrorManager.MESSAGE_CRC_EXCEPTION);
			} catch (Exception e) {
			    e.printStackTrace();
			    errorManager.error("PB messageHandler ", e,
				    ErrorManager.MESSAGE_EXCEPTION);
			} finally {
			...
			errorManager.error(
				"MessageQueueManager messageHandler null",
				new Exception("performRequest error"),
				ErrorManager.MESSAGE_ERROR);
		    }
		}
	    } catch (InterruptedException e) {
		errorManager.error("PB messageHandler ", e,
			ErrorManager.MESSAGE_INTERRUPTED_EXCEPTION);
	    }
	} while (alive && res);
    }
    ...
    public void setOpenIGT_Queue(
	    ConcurrentLinkedQueue<MessageHandler> openIGT_Queue) {
