...
public class BytesArray {
    ...
    private static final long[] CRC_TABLE = { 0x0000000000000000L,
    ...
    /*
     * for (int i = 0; i < 0x100; i++) { long v = i; for (int j = 0; j < 8; j++)
     * { // is current coefficient set? if ((v & 1) == 1) { // yes, then assume
     * it gets zero'd (by implied x^64 // coefficient of dividend) // and add
     * rest of the divisor v = (v >>> 1) ^ crc64Polynomial; } else { // no? then
     * move to next coefficient v = (v >>> 1); } } // Log.debug("0x" +
     * Long.toHexString(v)); CRC_TABLE[i] = v; }
     */
	...
	int tableIndex = 0;
	while (i < len) {
	    char unsignedCharCrc56 = (char) ((crc >> 56) & 0xFF);// Java does
								 // not support
								 // unsigned
								 // data types
								 // so this is
	    ...
	    unsignedCharBuf = (char) (unsignedCharBuf & 0xFF); // Java does not
							       // support
							       // unsigned data
							       // types so this
							       // is workaround
	...
	/*
	 * int i=0; while (i<len){ ByteBuffer b =
	 * ByteBuffer.allocate(Long.SIZE); b.putLong(crc >> 56); byte lastByte =
	 * b.get(16); //get Last byte in Long CRC
	 * 
	 * int tableIndex = Math.abs(buffer[i] ^ Math.abs(lastByte)); crc =
	 * CRC_TABLE[tableIndex] ^ (crc << 8); i++; } return crc;
	 */
	// Log.debug("crc len " + len);
	/*
	 * for (int i = 0; i < len; i++) { crc = next_crc(crc, buffer[i]); //
    ...
    /*
     * public long crc64(byte[] buffer, int len, long crc) { int index=1; //
     * ByteList b = new ByteList(buffer); while (len > 0) { crc =
     * CRC_TABLE[buffer[index-1]^(byte)(crc >> 56)]^(crc << 8); index++; len--;
     * } return crc; }
     */
