...
public class TransformMessage extends OpenIGTMessage {
	...
	// System.out.println("Transform: Calculated CRC=" + calculated_crc +
	// "REceived CRC=" + recvd_crc );
    }

    public TransformMessage(String deviceName, double[] positionAray,
	...
	Log.debug("Body size: " + getBody().length + " date size: "
    ...
    /**
     *** To create body from image_header and image_data SetTransformData must
     * have called first
     * 
     *** 
    ...
    /**
     *** To create transform_data from image characteristics and to get the byte
     * array to send
     * 
     * @param origin
     * @param normals
	...
	String transformString = "TRANSFORM Device Name           : "
		+ getDeviceName();
	for (int i = 0; i <= 3; i++)
	    for (int j = 0; j <= 3; j++)
		transformString = transformString.concat(Double
			.toString(matrix[i][j]));
