...
public class StatusMessage extends OpenIGTMessage {
    ...
    /**
     *** To create body from image_header and image_data SetStatusData must have
     * called first
     * 
     *** 
    ...
    /**
     *** To create status_data from server or client status and to get the byte
     * array to send
     * 
     * @param status
     *** 
	...
	this.errorName = bytesArray.getString(IGTL_STATUS_ERROR_NAME_LENGTH); // char
									      // 20
	// Control status_data.length-IGTL_STATUS_HEADER_SIZE-1 == '\0' ???
	this.statusString = bytesArray.getString(status_data.length
		- IGTL_STATUS_HEADER_SIZE);
	this.status = new Status((int) code, (int) subCode, errorName,
		statusString);
    }
	...
	String statusString = "STATUS Device Name           : "
		+ getDeviceName();
