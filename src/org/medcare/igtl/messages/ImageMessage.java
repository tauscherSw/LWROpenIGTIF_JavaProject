...
public class ImageMessage extends OpenIGTMessage {
	...
	return (int) (subDimensions[0] * subDimensions[1] * subDimensions[2]
		* 1 * 1); // assuming pixel size is 1 bytes and number of
			  // scalarcompents as 1

    }

    /**
     *** To create body from image_header and image_data SetImageHeader and
     * SetImageData must have called first
     * 
     *** 
    ...
    /**
     *** To create image_header from image carateristics and to get the byte array
     * to send
     * 
     * @param version
     * @param imageType
     ...
     */
    public byte[] setImageHeader(long version, long imageType, long scalarType,
	    long endian, long coordinate_type, long dimensions[],
	    double origin[], double normals[][], long subOffset[],
	    long subDimensions[]) {
	...
	// We don't test the result, subvolume is supposed to have been tested
	// during the byte array creation
     ...
     * @return true if Ok false if not
     */
    public boolean setSubVolume(int dimi, int dimj, int dimk, int offi,
	    int offj, int offk) {
	subDimensions = new long[3];
	subOffset = new long[3];
	// make sure that sub-volume fits in the dimensions
     ...
     *** 
     */
    public void setMatrix(double spacing[], double origin[], double norm_i[],
	    double norm_j[], double norm_k[]) {
	...
	String imageString = "IMAGE Device Name           : " + getDeviceName()
		+ "\n";
	imageString = imageString + "Scalar Type           : "
		+ getScalarType() + "\n";
	imageString = imageString + "Dimensions            : (" + dimensions[0]
		+ ", " + dimensions[1] + ", " + dimensions[2] + ")\n";
	imageString = imageString + "Spacing            : (" + spacing[0]
		+ ", " + spacing[1] + ", " + spacing[2] + ")";
	imageString = imageString + "Sub-Volume dimensions : ("
		+ subDimensions[0] + ", " + subDimensions[1] + ", "
		+ subDimensions[2] + ")\n";
	imageString = imageString + "Sub-Volume offset     : (" + subOffset[0]
		+ ", " + subOffset[1] + ", " + subOffset[2] + ")\n";
	return imageString;
    }
}
