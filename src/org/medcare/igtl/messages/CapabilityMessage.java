...
/**
 *** This class create an Capability object from bytes received or help to
 * generate bytes to send from it
 * 
...
public class CapabilityMessage extends OpenIGTMessage {
    ...
    /**
     *** To create body from image_header and image_data SetCapabilityData must
     * have called first
     * 
     *** 
	...
	System.arraycopy(capability_data, 0, getBody(), 0,
		capability_data.length);
	setHeader(new Header(VERSION, "CAPABILITY", deviceName, getBody()));
	return getBody();
    }

    /**
     *** To create capability_data from capabilityList and to get the byte array
     * to send
     * 
     * @param capabilityList
     *            : Capability ArrayList this ArrayList can be created by
     *            MessageHandler or by ResponseHandler
     *** 
	...
	for (int len = capability_data.length; len >= IGTL_CAPABILITY_TYPE_LENGTH; len -= IGTL_CAPABILITY_TYPE_LENGTH) {
	    this.capabilityList.add(bytesArray
		    .getString(IGTL_CAPABILITY_TYPE_LENGTH)); // char 12
	}
    }
    ...
    /**
     *** To set Capability ArrayList this ArrayList can be created by
     * MessageHandler or by ResponseHandler
     * 
     * @param capabilityList
     *** 
	...
	String capabilityString = "CAPABILITY Device Name           : "
		+ getDeviceName();
	Iterator<String> it = capabilityList.iterator();
	while (it.hasNext()) {
	    capabilityString = capabilityString.concat((String) it.next());
	}
