...
public class LWRVirtualFixtures implements LWRState {
	...
	public void CalcControlParam(LWRStatemachine lwrStatemachine) {
		...
		if(lwrStatemachine.InitFlag){
		 ...
		 if(this.VFtype == 1){
			...
			//T_Base_cone = MatrixTransformation.of(VirtualFixture_ap, Matrix.ofRowFirst(x_axis.getX(), x_axis.getY(), x_axis.getZ(), y_axis.getX(),  y_axis.getY(), y_axis.getZ(), VirtualFixture_n.getX(), VirtualFixture_n.getY(), VirtualFixture_n.getZ()));
			T_Base_cone = MatrixTransformation.of(VirtualFixture_ap, Matrix.ofRowFirst(1, 0, 0, 0, 1, 0, 0, 0, 1));
			//curCartPoseCOcone = T_Base_cone.applyTo(curMsrCartPose.getTranslation()).invert();
			...
			/*double dphi =  Math.acos( curCartPoseCOcone.dotProduct(Vector.of(0,0,1))/curCartPoseCOcone.length()) - VirtualFixture_phi/2 ;
			double psi= Math.atan2(curCartPoseCOcone.getY(),curCartPoseCOcone.getX());
			double l_xy= Math.sqrt(1-Math.pow(Math.sin(VirtualFixture_phi/2), 2));
			Vector x_cone=Vector.of(l_xy*Math.sin(psi), l_xy*Math.cos(psi),  Math.sin(VirtualFixture_phi/2));
			double xconeLength = curCartPoseCOcone.dotProduct(x_cone)/(Math.cos(VirtualFixture_phi/2)*curCartPoseCOcone.length());
			normvec=curCartPoseCOcone.invert().add(x_cone.normalize().multiply(xconeLength));
			...
			double sg= 2*zg;
			double tiefe = zg-curCartPoseCOcone.getZ();
			double lr;*/
			
			if(curCartPoseCOcone.length() < (awaredist) && curCartPoseCOcone.length()>10)
			{
				...
				normvec = T_cone_Base.getRotation().applyTo(v_temp.normalize()).normalize(); 
			}
		...
		if (distance < awaredist)
		{
			...
			}else if (distance >=0)	// im zulässigen Bereich
			{ 	
				
				
				if (distance-last_distance <= 0) // EE nähert sich an die Grenze	
				{
		...
		//Vector TransStiffVal_tool = lwrStatemachine.curPose.getRotation().getMatrix().multiply(aTransStiffVal);
	...
	public void SetACKPacket(LWRStatemachine lwrStatemachine) {
		...
		lwrStatemachine.AckIGTmessage = String;// TODO Automatisch generierter Methodenstub
	}
	...
	public void InterpretCMDPacket(LWRStatemachine lwrStatemachine) {
		// TODO Automatisch generierter Methodenstub
		boolean Error=false;
		if(lwrStatemachine.CmdIGTmessage.getHeader().getDataType().equals("STRING")){
			String CMD_String;
			try {
				CMD_String = new String(lwrStatemachine.CmdIGTmessage.getBody(),4,lwrStatemachine.CmdIGTmessage.getBody().length-4,"US-ASCII");
