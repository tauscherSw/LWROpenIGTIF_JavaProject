...
/**
 * The LWRError is a save State the robots is automatically set in if an error occurs. In this state the cartesian stiffness is set to a value of  100 and a moving Virtual plane is defined in the xy-plane of the robotbase coordinatesystem.
 * Thereby only movements with an increasing z Value are possible. 
 * 
 ...
 */
	 ...
	 */
	 public double get_stiffness_value_remove( double dist, double awaredist)	//Berechnung der Dämpfung bei Entfernung von der Grenze
	 { 	
	...
	public void CalcControlParam(LWRStatemachine lwrStatemachine) {
		...
		if (distance < safetydist)
		{
			...
			if (distance >=0)	// im zulässigen Bereich
			{ 	
				
				
				if (distance-last_distance <= 0) // EE nähert sich an die Grenze	
				{
					StiffVal = get_stiffness_value_approach(distance, safetydist);
				...
				lwrStatemachine.cmdPose = MatrixTransformation.of(curPosition, lwrStatemachine.curPose.getRotation());
			}
			else 				// im gesperrten Bereich
			{
				StiffVal = 5000;
				lwrStatemachine.cmdPose  = MatrixTransformation.of(curPosition.subtract(normvec.multiply(distance)), lwrStatemachine.curPose.getRotation());
			}
		}
		else 
	...
	public void SetACKPacket(LWRStatemachine lwrStatemachine) {
		...
		StatusMessage Status = new StatusMessage(ACKname, lwrStatemachine.ErrorCode, 0, lwrStatemachine.ErrorMessage);
