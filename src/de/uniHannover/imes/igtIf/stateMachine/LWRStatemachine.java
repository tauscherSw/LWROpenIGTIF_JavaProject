...
public class LWRStatemachine {
	...
	/**
	  * Current enum for the State Machine status {IDLE, GravComp, VirtualFixtures, PathImp, MovetoPose}possible client states
      */
     public static enum LWRStatus {IDLE, GravComp, VirtualFixtures, PathImp, MovetoPose}; //possible client states
	...
	/**
	 * Flag to identify if the Registration points were send successfully to the State Control.
	 */
	...
	/**
	 * Flag to identify if the StateMachine should be Stopped in the fastest.Attention the programm will stop without changing state.
	 */
	...
	public void CheckTransitionRequest(){
		...
		if(this.CmdIGTmessage.getHeader().getDataType().equals("STRING")){
			String CMD_String;
			try {
				CMD_String = new String(this.CmdIGTmessage.getBody(),4,this.CmdIGTmessage.getBody().length-4,"US-ASCII");
				...
				if(RobotState.name().equalsIgnoreCase(CMD_Array[0])){
					...
					if( CMD_Array[0].contentEquals("IDLE")){ 
						...
						if (RobotState==LWRStatus.IDLE || RobotState == LWRStatus.VirtualFixtures || RobotState == LWRStatus.PathImp ){
							...
							this.ErrorMessage = ("Transition from State " + RobotState.toString() + " to " + CMD_Array[0] +" is not allowed!");		
							ErrorFlag = true;
							this.ErrorCode = 10;
						}
					...
					//Now checking if the command to stop the State machine (Quit, End, Shutdown) was received	
						...
						this.ErrorMessage = "Unexpected COMMAND recieved! See the list opf supported Commands (received: " + CMD_Array[0] + ")" ; 
					}
