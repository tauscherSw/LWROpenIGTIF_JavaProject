 ...
 * developed at the WPI. For Further information see WPI Homepage.
 * The LWRStateMachineInterface Thread is operated with a 20ms cycle. An Example of the use of this class for the 
 * communication with a State Control Software e.g. 3D Slicer see imesStateApllication. According of the datatype of the OpenIGTMessage
 * the data is packed and send to the state control. Supported datattypes are "STATUS", "STRING" and "TRANSFORM". Later on "POINT" will be supported.
...
public class LWRStateMachineInterface extends Thread {
	 ...
	 */
     ...
     /**
      * Current enum for the client status {CONNECTED, DISCONNECTED }possible client states
      */
    ...
    /**
     * port number for the communication with state control. Possible ports 49001 - 49005 
     */
    ...
    **/
	...
	//                    Log.debug("New Body: "+b);
	          ...
	          if( 2 == header.getDeviceName().split("_").length){
		          ...
		          if(UIDdelay ==0){
		        	  UIDrepeat ++;
		        	  ErrorMessage= "State machine interface: UID has not changed for the " + UIDrepeat + ". time!! Check state control!";
		        	  ErrorFlag = true;
		          }else if(UIDdelay>1){
		        	  UIDmiss++;
		        	  ErrorMessage= "State machine interface: missed UID!!(miss count: " + UIDmiss + ")";
		        	  ErrorFlag = true;
	          ...
	          if (messageType.equals("STRING")){
	        	  ...
	        	  String body = new String(bodyBuf,4,bodyBuf.length-4,"US-ASCII");
	          }else if (messageType.equals("TRANSFORM")){
	        	  try {
						TransformMessage Transform = new TransformMessage(header, bodyBuf);
						Transform.Unpack();
						double[][] R_tmp = Transform.getRotationMatrixArray();
						double [] t_tmp =Transform.getPosition();
						com.kuka.roboticsAPI.geometricModel.math.Matrix R=com.kuka.roboticsAPI.geometricModel.math.Matrix.ofColumnFirst( R_tmp[0][0],  R_tmp[1][0],  R_tmp[2][0],  R_tmp[0][1],  R_tmp[1][1],  R_tmp[2][1],  R_tmp[0][2],  R_tmp[1][2],  R_tmp[2][2]);
						
						System.out.println("Transform:" + Transform.getRotationMatrixArray());
						MatrixTransformation T= MatrixTransformation.of(Vector.of(t_tmp[0], t_tmp[1], t_tmp[2]),R );
